...
class Contextual_Attention_Module(nn.Module):
    ...
    def forward(self, f, b, mask=None, ksize=3, stride=1,
        ...
        k = fuse_k
        scale = softmax_scale
        fuse_weight = Variable(torch.eye(k).view(1, 1, k, k)).to(self.device) #cuda(self.device) # 1 x 1 x K x K
        y_test = []
        for xi, wi, raw_wi in zip(f_groups, w_groups, raw_w_groups):
            ...
            # conv for compare
            wi = wi[0]
            escape_NaN = Variable(torch.FloatTensor([1e-4])).to(self.device) #cuda(self.device)
        ...
        h_add = Variable(torch.arange(0,float(bs[2]))).to(self.device).view([1, 1, bs[2], 1])  #cuda(self.device).view([1, 1, bs[2], 1])
        h_add = h_add.expand(bs[0], 1, bs[2], bs[3])
        w_add = Variable(torch.arange(0,float(bs[3]))).to(self.device).view([1, 1, 1, bs[3]])  #cuda(self.device).view([1, 1, 1, bs[3]])
